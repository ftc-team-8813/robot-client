...
def draw(img, packet):
    ...
    # 0x03: Line     <fill=uint32> <width=uint8> <join=uint8> <segments=uint8> <x0=uint16 y0=uint16> <x1=uint16 y1=uint16>...
    # 0x04: Pieslice <x0=uint16> <y0=uint16> <x1=uint16> <y1=uint16> <stroke=uint32> <fill=uint32> <width=uint8>
    ...
    while ptr < len(packet):
        ...
        if cmd == 0x00:
            ...
            fill, width, join, segment_ct = struct.unpack('>IBBB', packet[ptr:ptr+0x07])
            ptr += 0x07
            segments = []
